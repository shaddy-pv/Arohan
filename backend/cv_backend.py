...
import os, cv2, time, base64, pickle, threading, requests, uuid
import numpy as np
...
def _enhance(frame: np.ndarray) -> np.ndarray:
    ...
    if   m < 80:  out = cv2.LUT(out, _LUT_16)
    elif m < 120: out = cv2.LUT(out, _LUT_13)
...
class WhatsAppSender:
    ...
    def send(self, alert_type: str, severity: str, summary: str,
        ...
        if details:
            if details.get('location'):       msg += f"📍 Location: {details['location']}\n"
        ...
        msg += f"\n{summary}\n"
        msg += f"━━━━━━━━━━━━━━━━━\n"
        msg += f"_AROHAN Safety Monitoring System_"
    ...
    def call(self, alert_type: str, summary: str, recipient: Optional[str] = None):
        ...
        try:
            ...
            print(f"Voice call initiated: {alert_type} -> {to}  SID {call.sid}")
...
class IoTMonitor:
    ...
    THRESHOLDS = {
        'flame':       {'field': 'flame',       'op': 'bool_true',  'severity': 'critical',
                        'type': 'Fire Detected',
                        'msg': '🔥 Flame sensor triggered — immediate evacuation required!'},
        'mq2':         {'field': 'mq2',         'op': '>',  'value': 700, 'severity': 'high',
                        'type': 'Gas Leak',
                        'msg': '⚠️ Dangerous gas levels detected'},
        'mq135':       {'field': 'mq135',       'op': '>',  'value': 900, 'severity': 'medium',
                        'type': 'Poor Air Quality',
                        'msg': 'Air quality degraded'},
        'temperature': {'field': 'temperature', 'op': '>',  'value': 40,  'severity': 'medium',
                        'type': 'High Temperature',
                        'msg': '🌡️ Temperature exceeds safe limit'},
        'hazardScore': {'field': 'hazardScore', 'op': '>',  'value': 60,  'severity': 'high',
                        'type': 'High Hazard Level',
                        'msg': '☢️ Hazard score critical'},
        'motion':      {'field': 'motion',      'op': 'bool_true',  'severity': 'low',
    ...
    def start(self):
        if not FIREBASE_AVAILABLE:
            ...
            return
        self._run = True
        threading.Thread(target=self._loop, daemon=True, name='iot-monitor').start()
    ...
    def _check(self, data: dict):
        ...
        if emergency_active and not prev_emergency:
            ...
            self._wa.send(
                ...
                details={'location': 'Command Center'}
            )
            # Voice Call
            threading.Thread(
                target=self._wa.call,
                args=("Manual Emergency", "Emergency mode has been manually activated from the command center. All safety protocols are engaged."),
                daemon=True
            ).start()
            # Auto-dispatch Rover
            try:
                fb_db.reference('ronin/rover/control').update({'mode': 'auto', 'direction': 'forward', 'speed': 50})
...
class RoverControlBridge:
    ...
    def start(self):
        ...
        threading.Thread(target=self._direct_control_loop, daemon=True, name='rover-direct').start()
        # Only start Firebase polling if available
        if self._fb_ok:
            threading.Thread(target=self._control_loop, daemon=True, name='rover-ctrl').start()
        threading.Thread(target=self._status_loop, daemon=True, name='rover-status').start()
        ...
        while self._run:
            try:
                ...
                if emergency and not self._last_emergency:
                    ...
                    self._last_dir = 'stop'
                    try:
                        ref.update({'emergency': False, 'direction': 'stop', 'speed': 0})
                    except Exception:
                        pass
    ...
    def _check_mission(self):
        ...
        try:
            ...
            # When mission is dispatched from frontend and rover is IDLE, start mission
...
class CVBackend:
    ...
    def _init_fb(self):
        ...
        if not os.path.exists(p):
            print(f"  Firebase credentials not found: {p} -- Firebase disabled")
            FIREBASE_AVAILABLE = False
            return
        try:
    ...
    def _analyze(self, frame):
        ...
        for i,r in enumerate(results):
            ...
            a=RoverAlert(id=f"a_{int(time.time()*1000)}_{i}_{uuid.uuid4().hex[:8]}",type=tp,message=msg,
            ...
            def gen():
                ...
                while True:
                    try:
                        ...
                        ok,enc=cv2.imencode('.jpg',hud,[cv2.IMWRITE_JPEG_QUALITY,92])
                        if ok: yield b'--frame\r\nContent-Type: image/jpeg\r\n\r\n'+enc.tobytes()+b'\r\n'
    ...
    def _stream_alert(self,face,all_faces,ann,fc):
        ...
        url=f"{BACKEND_URL}/static/snapshots/{fname}"
        a=RoverAlert(id=f"a_{int(time.time()*1000)}_{fc}_{uuid.uuid4().hex[:8]}",type=tp,message=msg,
    ...
    def _accident(self,frame,ts):
        ...
        a=RoverAlert(id=f"a_{int(time.time()*1000)}_{uuid.uuid4().hex[:8]}",type='ACCIDENT',
